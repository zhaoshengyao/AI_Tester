# 阶段 06：API 缺陷清单（IPD 管理系统，批次 20260812-134842）

## 1. 当前阶段
API 自动化缺陷登记（接口契约/文档一致性 & 接口契约/实现一致性 两类分开）

## 2. 输入文件
- `projects/ipd/docs/test-runs/20260812-134842/raw/api-results/pytest-full-api-stdout.txt`（缺陷 stdout 证据源）
- `projects/ipd/docs/cases/API测试用例-评审版.md`
- 需求分析阶段 §KAN §M3 §WF 对应功能/命名/参数名的"产品/研发/测试口径预期"

## 3. 产出文件
- `projects/ipd/docs/test-runs/20260812-134842/defects/API缺陷清单.md`（本文件）
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | API-DOC-001 | 接口契约/文档一致性 | 中 | 见 stdout | API-001 | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 研发/产品确认最终命名或参数名，更新文档或兼容旧名。 |
| 2 | API-IMP-001 | 接口契约/实现一致性 | 高 | 见 stdout | API-004 分页参数未生效 | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 研发修复实现或补充过滤参数名；修复后按原用例复测。 |
| 3 | API-IMP-002 | 接口契约/实现一致性 | 中 | 见 stdout | API-003-A3（AMB-01 状态2/3） | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 研发修复实现或补充过滤参数名；修复后按原用例复测。 |
| 4 | API-IMP-003 | 接口契约/实现一致性 | 中 | 见 stdout | API-005 R10(AMB-03) | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 研发修复实现或补充过滤参数名；修复后按原用例复测。 |
| 5 | DEF-BIZ-001 | 接口契约/业务校验或权限越权一致性 | 高 | 见 stdout | WR-11-HIGH | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 按 AGENTS.md 责任方（研发/产品）自主决定是否修复；复测用例对应 stdout 缺陷标题中的 WR/API 编号。 |
| 6 | DEF-BIZ-002 | 接口契约/业务校验或权限越权一致性 | 中 | 见 stdout | WR-14 | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 按 AGENTS.md 责任方（研发/产品）自主决定是否修复；复测用例对应 stdout 缺陷标题中的 WR/API 编号。 |
| 7 | DEF-BIZ-003 | 接口契约/业务校验或权限越权一致性 | 中 | 见 stdout | WR-15 | `20260812-134842/raw/api-results/pytest-full-api-stdout.txt` | 按 AGENTS.md 责任方（研发/产品）自主决定是否修复；复测用例对应 stdout 缺陷标题中的 WR/API 编号。 |
...
```
[接口契约/权限一致性缺陷 WR-11-HIGH] 赵经理POST /idmp/project/approve/2087416041007173633 返回 code=200！可能为 IDOR 高危越权写！resp={"code": 200, "msg": "操作成功", "data": null}
[接口契约/权限一致性缺陷 WR-11-HIGH] 赵经理POST /idmp/project/phase/2087416041007173633?toPhase=closed 返回 code=200！可能为 IDOR 高危越权写！resp={"code": 200, "msg": "操作成功", "data": null}
[WR11] attempt log: [{"method": "PUT", "path": "/idmp/project", "HTTP": 200, "biz_code": 500}, {"method": "POST", "path": "/idmp/project/submit/2087416041007173633", "HTTP": 200, "biz_code": 500}, {"method": "POST", "path": "/idmp/project/approve/2087416041007173633", "HTTP": 200, "biz_code": 200}, {"method": "POST", "path": "/idmp/project/phase/2087416041007173633?toPhase=closed", "HTTP": 200, "biz_code": 200}, {"method": "PUT", "path": "/idmp/project/2087416041007173633", "HTTP": 200, "biz_code": 405}, {"method": "DELETE", "path": "/idmp/project/2087416041007173633", "HTTP": 200, "biz_code": 500}]
```
